...
<div class="container mt-5 mb-5 border p-5 bg-primary ">
    <div><h4 class="text-center text-light">Record will be saved</h4></div>
    <div class="btns text-right">
        <form id="log" method="get" action="{{url('/')}}">
            @CSRF
            <button type="submit" class="btn btn-success  border-bottom"> Add New User</button>
        </form>
        <form id="log" method="get" action="{{url('/Display')}}">
        @CSRF
            <button type="submit" class="btn btn-success border-bottom"> Show All User</button>
        </form>
    </div>
</div>
